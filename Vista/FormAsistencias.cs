...
namespace Vista
{
    public partial class FormAsistencias : Form
    {
        ...
        private void ActualizarGrillaAsistencias()
        {
            dgvAsistencias.DataSource = null;

            // Recupero el libro de asistencias del alumno.
            var libroDeAsistencias = controladoraLibrosDeAsistencias.RecuperarLibroAlumno(alumno);

            // Creo una lista con los datos necesarios para el dgvAsistencias.
            var datosGrilla = libroDeAsistencias.Asistencias.Select(asistencia => new
            {
                Fecha = asistencia.Fecha.ToShortDateString(),
                TipoAsistencia = asistencia.Tipo1.ToString(),
                Trimestre = asistencia.Trimestre.NumTrimestre
            }).ToList();
